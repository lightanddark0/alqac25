...
try:
    ...
    print("Chạy: pip install rank-bm25 transformers torch scikit-learn underthesea requests sentence-transformers faiss-cpu")
    exit(1)
...
class Stage3_LLMGenerator:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "AITeamVN/Vi-Qwen2-7B-RAG", local_model_dir: Optional[str] = None):
        self.api_key = api_key
        self.model_name = model or local_model_dir or "AITeamVN/Vi-Qwen2-7B-RAG"
    ...
    def _try_load_local_model(self):
        ...
        try:
            ...
            logger.info(f"Đang tải model local từ {self.model_name}...")
            ...
            logger.error(f"Không thể load model local từ {self.model_name}: {e}")
            self.local_model = None
            self.local_tokenizer = None

    def _build_prompt(self, question: ALQACQuestion, context: str) -> str:
        return self.template.format(context=context, question=question.text)

    def generate_answer(self, question: ALQACQuestion, retrieved_docs: List[RetrievalResult]) -> Dict[str, Any]:
        if self.local_model and self.local_tokenizer:
            ...
            try:
                ...
                return {
                    ...
                    'confidence': 0.8,
                    'reasoning': 'Generated by Vi-Qwen2-7B-RAG',
...
class EmbeddingRetriever:
    """Retriever sử dụng truro7/vn-law-embedding"""
    def __init__(self, model_name: str = "truro7/vn-law-embedding", device: str = None):
        self.model_name = model_name
        self.device = device or ('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    def index_documents(self, documents: List[LegalDocument]):
        ...
        self.embeddings = self.model.encode(texts, batch_size=2, normalize_embeddings=True, show_progress_bar=True)
        ...
        for score, idx in zip(scores[0], indices[0]):
            if idx < len(self.documents):
                results.append(RetrievalResult(
                    ...
                    stage_scores={'embedding': float(score)}
                ))
        return results

class HybridRetriever:
    """Hybrid BM25 + Embedding Retriever"""
    def __init__(self, bm25_retriever: Stage1_BM25Retriever, emb_retriever: EmbeddingRetriever, weight_bm25: float = 0.5, weight_emb: float = 0.5):
    ...
    def index_documents(self, documents: List[LegalDocument]):
        ...
        self.emb_retriever.index_documents(documents)

    def retrieve(self, query: str, top_k: int = 10) -> List[RetrievalResult]:
        bm25_results = self.bm25_retriever.retrieve(query, top_k=top_k*2)
        emb_results = self.emb_retriever.retrieve(query, top_k=top_k*2)
        # Gộp kết quả theo doc_id
        score_dict = {}
        doc_map = {}
        for r in bm25_results:
            score_dict[r.document_id] = {'bm25': r.score, 'embedding': 0.0, 'doc': r.document}
            doc_map[r.document_id] = r.document
        for r in emb_results:
            if r.document_id in score_dict:
                score_dict[r.document_id]['embedding'] = r.score
            else:
                score_dict[r.document_id] = {'bm25': 0.0, 'embedding': r.score, 'doc': r.document}
                doc_map[r.document_id] = r.document
        # Kết hợp điểm
        hybrid_results = []
        for doc_id, v in score_dict.items():
            combined = self.weight_bm25 * v['bm25'] + self.weight_emb * v['embedding']
            hybrid_results.append(RetrievalResult(
                document_id=doc_id,
                score=combined,
                document=v['doc'],
                stage_scores={'bm25': v['bm25'], 'embedding': v['embedding'], 'hybrid': combined}
            ))
...
class MultiStageRetrievalSystem:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.embedding_retriever = EmbeddingRetriever(
            model_name=self.config.get('embedding_model', 'truro7/vn-law-embedding'),
            device=self.config['device']
        )
        self.hybrid_retriever = HybridRetriever(
            self.stage1,
            self.embedding_retriever,
            weight_bm25=self.config.get('hybrid_weight_bm25', 0.5),
            weight_emb=self.config.get('hybrid_weight_emb', 0.5)
        )
        ...
        self.stage3 = Stage3_LLMGenerator(
            ...
            model=self.config.get('llm_model', 'AITeamVN/Vi-Qwen2-7B-RAG'),
            local_model_dir=self.config.get('llm_local_model_dir', 'AITeamVN/Vi-Qwen2-7B-RAG')
        )
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        return {
            ...
            'llm_api_key': None,
            'llm_model': 'AITeamVN/Vi-Qwen2-7B-RAG',
            'llm_local_model_dir': 'AITeamVN/Vi-Qwen2-7B-RAG',
            'embedding_model': 'truro7/vn-law-embedding',
            'hybrid_weight_bm25': 0.5,
            'hybrid_weight_emb': 0.5,
            'retriever_type': 'hybrid'  # 'bm25', 'embedding', 'hybrid'
        }
    ...
    def build_index(self):
        ...
        if self.config.get('retriever_type', 'hybrid') == 'hybrid':
            self.hybrid_retriever.index_documents(self.legal_corpus)
        elif self.config.get('retriever_type') == 'embedding':
            self.embedding_retriever.index_documents(self.legal_corpus)
        else:
    ...
    def process_single_question(self, question: ALQACQuestion) -> Dict[str, Any]:
        ...
        if self.config.get('retriever_type', 'hybrid') == 'hybrid':
            ...
            stage1_results = self.embedding_retriever.retrieve(
                question.text,
                top_k=self.config['bm25_top_k']
            )
        else:
...
def run():
    ...
    questions_file = 'alqac25_test_split.json'
    try:
        system.load_data(questions_file)
        system.build_index()
        results = system.process_all_questions()
        system.save_results(results, 'alqac_hybrid_results.json')
        print("\nHệ thống hybrid retrieval đã chạy thành công!")
        print("Kết quả đã được lưu vào 'alqac_hybrid_results.json'")
    except Exception as e:
        logging.error(f"Lỗi khi chạy hybrid retrieval: {e}")
        print(f"Lỗi: {e}")
