...
class MultiStageRetrievalSystem:
    ...
    def _print_summary(self, metrics: Dict[str, Dict[str, float]]):
        ...
        if 'task1' in metrics:
            ...
            print(f"  Precision: {metrics['task1']['avg_precision']:.4f} ± {metrics['task1']['std_precision']:.4f}")
            print(f"  Recall:    {metrics['task1']['avg_recall']:.4f} ± {metrics['task1']['std_recall']:.4f}")
            print(f"  F2-score:  {metrics['task1']['avg_f2']:.4f} ± {metrics['task1']['std_f2']:.4f}")
...
def main():
    ...
    config = {
        ...
        'bert_model': 'vinai/phobert-base',
        'bert_top_k': 1,  # Giảm để phù hợp với máy yếu
        'bert_alpha': 0.3,
